...
def shap_sankey(df: pd.DataFrame, final_node: str = "root", val_col="value", cmap_name='coolwarm'):
...
def complete_shap_sankey(df, show_top_n=10, savename="complete_sankey", val_col='value', cmap_name="Reds"):
    ...
    def get_top_n(df, layer, n):
        ...
        s = l.groupby('source', as_index=False).mean(
    ...
    def get_node_colors(sources, df):
        ...
        for source in sources:
            ...
            if "Other connections" in source:
                ...
                intensity = source_df.groupby('source_w_other').mean()[
    ...
    def get_node_positions(feature_labels, df):
        ...
        for layer in layers:
            other_df = grouped_df.loc[grouped_df['source_w_other'].str.startswith(
                'Other')]
            other_value = other_df.groupby('source_w_other').mean().value[0]
            layer_df = grouped_df[grouped_df['source layer']
                                  == layer].sort_values(['value'], ascending=True)
