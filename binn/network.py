...
def get_mapping_to_all_layers(path_df, translation_df):
    graph = nx.from_pandas_edgelist(path_df, source='child', target='parent', create_using=nx.DiGraph())
    ...
    for translation in translation_df['input']:
        ...
        for id in ids:
            connections = graph.subgraph(nx.single_source_shortest_path(graph, id).keys()).nodes
...
class Network:
    ...
    def __init__(self,
        ...
        self.mapping = subset_on_proteins_in_ms_data(input_data, self.mapping, input_data_column, verbose)
    ...
    def build_network(self):
        ...
        net = nx.from_pandas_edgelist(self.pathways, 'parent', 'child', create_using=nx.DiGraph())
        # add root node
...
def complete_network(G, n_levels=4):
    ...
    for node in terminal_nodes:
        ...
        if distance <= n_levels:
            ...
            sub_graph = add_edges(sub_graph, node,
                                  diff)  # This just adds nodes if n_levels is longer than distance. It creates node_last_copy1, node_last_copy2 etc.
...
def subset_on_proteins_in_ms_data(input_df, translation_df, input_data_column, verbose=False):
    ...
    if verbose:
        ...
        print(f"Unique proteins in reactome df: {len(list(translation_df['input'].unique()))}")
